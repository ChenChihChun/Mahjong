...
- [x] R3-2: 加入「聽」按鈕 → 聽牌已自動偵測顯示，不需額外宣告按鈕
- [x] R3-3: 取消按鈕改為紅色醒目樣式（參考圖中紅色取消鈕）
- [x] R3-4: 頭像旁加語音/表情按鈕 → 簡化為表情符號快捷列
- [x] R3-5: 牌桌背景改為室內木質裝潢風格（參考第二張圖的木質背景）
- [x] 少了一開始的抓位流程、擲骰(骰莊) → 已改為隨機莊家
- [x] 出了牌後 電腦沒動作 → botBusy race condition fix
...
- [x] 遊戲結束後「再來一局」應正常運作 → 清除 interval/flags
- [x] 白板(5z)顯示改善 → 加底色和陰影
- [x] 電腦的吃/碰/花牌沒顯示 → 在 avatar 旁顯示 melds/flowers
- [x] 電腦乙位置的牌被遮住 → discard area z-index 修正
- [x] 圈數設定（1局/1圈/2圈/4圈）加入 lobby
- [x] 摸到的牌與手牌分隔顯示
- [x] 放槍計分修正（移除不合理的3x倍數）
- [x] 胡牌畫面顯示完整牌組（含鳴牌和花牌）
- [] bot 出牌核心邏輯仍需靠 stuck detector，應修復根本原因
...
- 每輪取第一個 `[ ]` 任務來做
- 完成後把 `[ ]` 改成 `[x]`
- 如果所有任務都是 `[x]`，則持續優化 去網路上找明星三缺一的麻將遊戲內容圖片，參考設計，找出與圖像中10處不同的地方 寫入待辦任務，進行遊戲遊玩寫入10個待改善問題，進行持續優化，繼續執行
