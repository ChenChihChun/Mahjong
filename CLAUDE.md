...
- [] 遊戲邏輯: bot智慧出牌改善（不打危險牌）
- [] UI: 圓形動作按鈕（參考c89ecf1bf349c1312494017fecdc62b3.jpg的吃/碰/槓/胡圓按鈕）
- [] UI: 每張可出牌顯示台數預覽
- [] UI: 聽牌玩家顯示「聽」標記
- [] UI: 風位標記顯示在牌桌對應位置
- [] 遊戲邏輯: 放槍三家付（點炮者包三家）選項
- [] 遊戲邏輯: 天聽判定
- [] UI: 牌桌中央顯示當前局風（東/南/西/北風局）


## 任務執行規則
- 每輪取第一個 `[ ]` 任務來做
- 完成後把 `[ ]` 改成 `[x]`
- 如果所有任務都是 `[x]`，則持續優化 盡量比照 C:\Users\USER\Pictures\maxresdefault.jpg、c89ecf1bf349c1312494017fecdc62b3.jpg，找出與圖像中10處不同的地方 寫入待辦任務 進行持續優化，繼續執行
